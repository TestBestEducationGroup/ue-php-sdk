...
class Connection
{
     ...
     * Default urlencoded media type.
     */
    const MEDIA_TYPE_MULTI = 'multipart/form-data';
    /**
     ...
     */
    public function authenticateBearer($apiKey)
    {
     ...
     * @return mixed
     */
    public function post_form($url, $body, $files_data)
    {
        $post_data = $body;
        if ($files_data) {
            foreach ($files_data as $key => $file_data) {
                ...
                if (count($file_data) > 1) {
                    $index = 0;
                    foreach ($file_data as $name => $content) {
                        $key_name = $key . '[' . $index . ']';
                        $post_data->$key_name = curl_file_create($content, NULL, $name);
                        ++$index;
                    }

                } else {
                    foreach ($file_data as $name => $content) {
                        $post_data->$key = curl_file_create($content, NULL, $name);
                    }
                }
            }
        }
        $this->initializeRequest();
        $this::addHeader("Content-type", $this::MEDIA_TYPE_MULTI);
        curl_setopt($this->curl, CURLOPT_CUSTOMREQUEST, 'POST');
        curl_setopt($this->curl, CURLOPT_URL, $url);
        curl_setopt($this->curl, CURLOPT_POST, true);
        curl_setopt($this->curl, CURLOPT_PUT, false);
        curl_setopt($this->curl, CURLOPT_HTTPGET, false);
        curl_setopt($this->curl, CURLOPT_TRANSFERTEXT, false);
